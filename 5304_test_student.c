...
struct student *append(struct student *next, struct student *newStudptr) {
  //! heap=showMemory(start=260, cursors=[end,newStudptr])
  next->next = newStudptr;  // linking
  next = newStudptr;        // create new next
  return (next);
}