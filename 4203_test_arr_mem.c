...
int main(void) {
  ...
  */
  ...
  /* The line you add should use the array cookie (and not the array dry) to
     ...
     in the variable totalDry. This time, use only one pair of brackets [..]
     each time you address the cookie array.
  */
  ...
  /* The lines you add should use the array cookie (and not the array wet) to
     ...
     You believe that any cookie recipe should use equal amounts of water and
     oil. Without changing the total amount of wet ingredients, update the
     values for water and oil, using only the array cookie (and not the array
     wet) so that these amounts will be equal. The easiest way to do so is to
     assign the value totalWet/2 to both the water and the oil entry. When
     addressing the array cookie, do not use any brackets at all this time.
  */
  **cookie = totalWet / 2;
  *((*cookie) + 1) = totalWet / 2;
